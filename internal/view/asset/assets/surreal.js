...
let surreal = (function () {
  let $ = {
    ...
    sugar(e) {
      if (e == null) {
        console.warn(`Surreal: Cannot use "${e}". Missing a character?`);
      }
    ...
    isSelector(selector = "", start = document, warning = true) {
      ...
      if (start.querySelector(selector) == null) {
        if (warning)
          console.warn(
            `Surreal: "${selector}" was not found. Missing a character? (. #)`,
          );
        return false;
      }
      return true; // Valid.
...
function pluginEffects(e) {
  ...
  // Add sugar
  e.fadeOut = (fn, ms, remove) => {
    return fadeOut(e, fn, ms, remove);
  };
  e.fade_out = e.fadeOut;
  e.fadeIn = (fn, ms) => {
    return fadeIn(e, fn, ms);
  };
  e.fade_in = e.fadeIn;
}
